...
function App() {
  //option 1 for Firebase
  const [dataList, setDataList] = useState([])
  const dataCollectionRef = collection(db, "namedata")
  useEffect(() => {
    const getList = async () => {
  ...
  const onNameChange = (event) => {
    ...
    let myList = [...DATASET]
    myList = myList.filter(item => item.toLowerCase().includes(event.target.value.toLowerCase()))
  ...
  return (
    <>
        <div className="container">
          <input type="text" placeholder="name" value={nameFilter}
            onChange={onNameChange}/>
          <button onClick={handleReset} className="btn-grad">Reset filter</button>
          <section>
